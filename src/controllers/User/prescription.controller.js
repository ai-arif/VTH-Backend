...
export const getAllPrescriptions = async (req, res) => {
  try {
    ...
    const appointments = await Appointment.find({ owner }).select({ _id: 1 });
    ...
    const prescriptions = await Prescription.find({ appointment: { $in: appointmentIds } })
      ...
      .populate({
        ...
        populate: [
          ...
          { path: "owner", select: "fullName phone address district upazila" },
          { path: "complaint", select: "complaint" },
          { path: "breed", select: "breed" },
          { path: "species", select: "name" }
        ],
      })
      .populate({ path: "prescribedBy", select: "fullName" })
      .populate("medicines")
      .lean();

    const count = await Prescription.countDocuments({ appointment: { $in: appointmentIds } });
