...
export const appointmentSSLPayment = async (req, res) => {
    ...
    try {
        ...
        const userData = await User.findOne({ phone: appointmentData?.phone }).select("-password");
        ...
        //to do
        let totalPayment = 700.00;

        const data = {
            total_amount: totalPayment,
            currency: "BDT",
            tran_id: transId, // use unique tran_id for each api call
            success_url: `http://localhost:5000/api/v1/user-appointment/payment/success/${id}?amount=${totalPayment}`,
            fail_url: `http://localhost:5000/api/v1/user-appointment/payment/fail/${id}`,
            cancel_url: `http://localhost:5000/api/v1/user-appointment/payment/fail/${id}}`,
...
export const appointmentSSLPaymentSuccess = async (req, res) => {
    try {
        ...
        const result = await Appointment.findByIdAndUpdate(id, { $set: { amount: amount, payment: "paid" } }, { new: true });

        res.redirect(`http://localhost:3001/payment/success?appointmentId=${req.params.id}`);
    }
...
export const appointmentSSLPaymentFail = async (req, res) => {
    ...
    try {
        res.redirect(`http://localhost:3001/payment/fail?appointmentId=${req.params.id}`);
    }
